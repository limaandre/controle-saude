...
export class SearchDiseaseComponent implements OnInit {
    ...
    parseData(data) {
        ...
        for (const key in data) {
            if (Object.prototype.hasOwnProperty.call(data, key)) {
                if (key !== 'idDisease' && key !== 'show' && key !== 'filter' && key !== 'idDoctor' && key !== 'date') {
                    newData.push(
                        { key, data: data[key] }
                    );
    ...
    async openData(data) {
        const id = data.idDisease;
        const formLabel = 'DISEASES.form';
        const headerText = 'DISEASES.textHeader';
        ...
        modal.onDidDismiss()
            .then((dateReturn: any) => {
                if (dateReturn?.data?.delete) {
                    this.appService.disease({ search: '' }, 'get').subscribe(async (response: any) => {
                        if (response.data) {
