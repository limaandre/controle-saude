...
class GiixCrudCode extends CrudCode {
	...
	public function generateActiveField($modelClass, $column) {
		...
		if ($column->isForeignKey) {
			...
			if($relatedModelClass == ''){
				...
				return '$this->widget(\'GalleryManager\', array(
				  \'gallery\' => $model->'.$nome_behavior.'->getGallery(),
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
				...
				|| strtoupper($column->dbType) == 'CHAR(1)') {
			return "echo \$form->checkBox(\$model, '{$column->name}')";
		} 
		elseif (strtoupper($column->dbType) == 'DATE') {
			return "\$this->widget('CJuiDateTimePicker',array(
								'model'=>\$model, //Model object
								'attribute'=>'{$column->name}', //attribute name
								'language' => 'pt',
		...
		elseif($column->size == 140){
			return "\$this->widget('application.extensions.Plupload.PluploadWidget', array(
			'model' => \$model,
			'attribute' => '{$column->name}',
		  ))";
		}
			...
			elseif($column->size == 120 || $column->size == 130)
	...
	public function generateDetailView($modelClass, $column) {
		Yii::import('ext.giix-core.giixModel.GiixModelCode'); 
		$model_code = new GiixModelCode();
		...
		if (!$column->isForeignKey) {
			if (strtoupper($column->dbType) == 'TINYINT(1)'
					...
					|| strtoupper($column->dbType) == 'BOOLEAN'
					|| strtoupper($column->dbType) == 'CHAR(1)') {
				return "<?=Util::formataTexto(\$model->$column->name ? 'Sim' : 'Não')?>";
			} 
			elseif(strtoupper($column->dbType) == 'BLOB'){
				return "<?=\$model->$column->name?>";
			}
			elseif($column->size == 120)
				return '<img style="margin-top:10px;" class="img-polaroid" src="<?php echo Yii::app()->request->baseUrl; ?>/<?=$model->'.$model_code->generateClassName($column->name).'->getAttachment(\'p\');?>" />';
			elseif($column->size == 130)
				return '<a style="margin-top:10px;" target="_blank" class="btn-link" href="<?php echo Yii::app()->request->baseUrl; ?>/<?=$model->'.$column->name.';?>" ><?=$model->'.$column->name.';?></a>';
			elseif($column->size == 140)
				return '<a style="margin-top:10px;" target="_blank" class="btn-link" href="<?php echo Yii::app()->request->baseUrl; ?>/uploads/'.$modelClass.'/<?=$model->'.$column->name.';?>" ><?=$model->'.$column->name.';?></a>';
			elseif($column->size == 150){
				Yii::import('ext.giix-core.giixModel.GiixModelCode'); 
				$model_code = new GiixModelCode();
				return '<?=Util::formataTexto($model->get'.$model_code->generateClassName($column->name).'())?>';
			}
			else
				return "<?=Util::formataTexto(\$model->$column->name)?>";
		} 
		else {
			// Find the relation name for this column.
			$relation = $this->findRelation($modelClass, $column);
			...
			return "<?=(\$model->{$relationName} !== null ? GxHtml::link(GxHtml::encode(GxHtml::valueEx(\$model->{$relationName})), array('{$relatedControllerName}/view', 'id' => GxActiveRecord::extractPkValue(\$model->{$relationName}, true)),array('class' => 'relational-link')) : null)?>";
		}
	}
...
EOM;
	...
	public function getCrudOrder($tableSchema){
		$comments = Metadata::getTableComments($tableSchema->name);
		return $comments[3] != "" ? $comments[3] : $tableSchema->primaryKey." desc";
	}
}