...
abstract class GxActiveRecord extends CActiveRecord {
	...
	/*public function deleteRecursive(){
		...
		if(count($relations) > 0){
			foreach($relations as $relation) 
			{
				...
				elseif(is_object($this->$relation)) 
				{
					echo get_class($this->$relation);
	...
	public function beforeDelete(){
		...
		if(!Yii::app()->user->isGuest){
			$garbage->idusuario = Yii::app()->user->obj->idusuario;
			$garbage->usuario_nome = Yii::app()->user->obj->nome;
		}
		else{
			$garbage->usuario_nome = 'app';
		}
		$garbage->data = date('Y-m-d H:i:s');
		$garbage->tabela = $this->tableSchema->name;
		
		$garbage->nome = $this->representingName();
		...
		$registro = Yii::app()->db->createCommand()
					...
					->where($this->tableSchema->primaryKey.'=:id', array(':id'=>$this->$id))
					->queryRow();
					
		
		
		$garbage->sql_insert = "INSERT INTO ".$this->tableSchema->name." (".implode(",",array_keys($registro)).") VALUES ('".implode("','",array_values($registro))."')";
		
		$garbage->dados = CJSON::encode($registro);	
		
		$garbage->hash = isset($_GET['hash']) ? $_GET['hash'] : $garbage->getHash();
	...
	public function getRelationData(){
		...
		if(count($relations) > 0){
			foreach($relations as $relation => $dados) {
				$type = $dados[0];
				if($type == self::HAS_MANY || $type == self::HAS_ONE || $type == self::MANY_MANY){
					if(is_array($this->$relation) && count($this->$relation) > 0){
						foreach($this->$relation as $relation_item){
							$array_relacionados[$relation_item->label(2)][] = $relation_item;
						}                
					} 
					elseif(is_object($this->$relation)) {
						$relation_item = $this->$relation;
						$array_relacionados[$relation_item->label(1)][] = $relation_item;
					}
				}
				
			}
		}
		return $array_relacionados;
	}
	
	public function relationsRecursiveDelete(){
		$relations = $this->relations();
		$relations_array = array();
		foreach($relations as $relation => $dados) {
			$type = $dados[0];
			if($type == self::HAS_MANY || $type == self::HAS_ONE)
				$relations_array[] = $relation;	
		}
	...
	public function deleteRecursive(){
		...
		$relations = $this->relationsRecursiveDelete();
	...
	public function representingName() {
		$rc = $this->representingColumn();
        return is_array($rc) ? $this->$rc[0] :  $this->$rc;
	}
}