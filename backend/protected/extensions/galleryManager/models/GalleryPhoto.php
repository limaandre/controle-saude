...
class GalleryPhoto extends CActiveRecord
{
	...
	{
		//dir_arquivo onde esta o original
		$dir_arquivos = 'gallery/';
		//dir_arquivo_upa onde sera enviado
		$dir_arquivos_upa = "www_agraria/extranet_2016/".$dir_arquivos;
		
		//save image in original size
		Yii::app()->image->load($path)->save($this->galleryDir . '/' . $this->getFileName('') . '.' . $this->galleryExt);
		
		//arquivo original
		$arquivo = $this->galleryDir . '/' . $this->getFileName('') . '.' . $this->galleryExt;
		//nome do arquivo
		$arquivo_nome = $this->getFileName('') . '.' . $this->galleryExt;
		/*Envia para o FTP o arquivo original*/
		$resultado_ftp = '';
		$resultado_ftp = UpaArquivos::fazUpload($arquivo_nome, $arquivo,$dir_arquivos_upa);
		if($resultado_ftp!='sucesso'){
			//faz um alert
			echo "<script>alert('".$resultado_ftp."');</script>";
		}
		...
		//arquivo original
		$arquivo = $this->galleryDir . '/_' . $this->getFileName($version) . '.' . $this->galleryExt;
		//nome do arquivo
		$arquivo_nome = '_'.$this->getFileName('') . '.' . $this->galleryExt;
		/*Envia para o FTP preview da galeria*/
		$resultado_ftp = '';
		$resultado_ftp = UpaArquivos::fazUpload($arquivo_nome, $arquivo,$dir_arquivos_upa);
		if($resultado_ftp!='sucesso'){
			//faz um alert
		...
		foreach ($this->gallery->versions as $version => $actions) {
			...
			//arquivo original
			$arquivo = $this->galleryDir . '/' . $this->getFileName($version) . '.' . $this->galleryExt;
			//nome do arquivo
			$arquivo_nome = $this->getFileName($version) . '.' . $this->galleryExt;
			/*Envia para o FTP as versoes*/
			$resultado_ftp = '';
			$resultado_ftp = UpaArquivos::fazUpload($arquivo_nome, $arquivo,$dir_arquivos_upa);
			if($resultado_ftp!='sucesso'){
				//faz um alert
