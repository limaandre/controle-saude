(function ($) {
    ...
    function galleryManager(el, options) {
        ...
        function createEditorElement(id, src, name, description) {
            var html = 
				'<div class="photo-editor">' +
					'<div class="row">' +
						'<div class="col-md-4 col-sm-12"><img class="img-thumbnail" src="' + htmlEscape(src) + '" alt=""/></div>' +
						...
						(opts.hasDesc
							...
							'<textarea name="photo[' + id + '][description]" rows="3" cols="40" class="form-control" id="photo_description_' + id + '">' + htmlEscape(description) + '</textarea>'
							: '') +
						'</div>' +
					'</div>' +
                '</div>';
        ...
        if (opts.hasName || opts.hasDesc)photoTemplate += '<span class="editPhoto btn btn-primary btn-sm"><i class="fa fa-pencil fa fa-white"></i></span> ';
        photoTemplate += '<span class="deletePhoto btn btn-danger btn-sm"><i class="fa fa-remove fa fa-white"></i></span>' +
