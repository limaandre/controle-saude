...
class AttachmentBehavior extends CActiveRecordBehavior {
    ...
    public $attribute_ext = '';
    public $attribute_delete = '';
    ...
    public function concatUrl() {
        $attrs = explode("/", $this->path);
        $file_name_parse = array_pop($attrs);
        $this->path = implode("/", $attrs);
        $this->Owner->{$this->attribute} = $this->getParsedPath($this->path) . '/' . $this->Owner->{$this->attribute};
    }

    public function getFileNameParse($path) {
        $attrs = explode("/", $path);
        $file_name_parse = array_pop($attrs);
    ...
    public function hasAttachment() {
        return is_file($this->Owner->{$this->attribute});
    }
    ...
    public function beforeSave($event) {
        ...
        if (!empty($this->attribute_delete) && $this->Owner->{$this->attribute_delete} == 1)
    ...
    public function afterSave($event) {
        ...
        if (!empty($attribute_delete) && $this->$attribute_delete == 1)
            return true;
        ...
        if (!is_null($file)) {
            ...
            $extensao = strtolower(end(explode('.', $this->file_extension)));
            if(in_array($extensao,$extensoes_invalidas)){
                throw new CException('Extensão inválida');
            }
            ...
            $this->Owner->saveAttributes(array($this->attribute => $this->getFileNameParse($path)));

            $attributes = $this->Owner->attributes;

            if ($this->attribute_size != "") {
                $this->Owner->saveAttributes(array($this->attribute_size => filesize($path)));
            }
            if ($this->attribute_type != "") {
                $this->Owner->saveAttributes(array($this->attribute_type => mime_content_type($path)));
            }
            if ($this->attribute_ext != "") {
                $this->Owner->saveAttributes(array($this->attribute_ext => $this->file_extension));
            }
